...
export default function OrderSummary() {
  ...
  const calculateSubtotal = () => {
    return cart.reduce((sum, item) => sum + item.quantities * 250, 0)
  }
    ...
    if (user && userInfo) {
      try {
        for (const item of cart) {
          const { fabric, shirt_type, sizes, quantities, status, shipping_address } = item
  ...
  const shippingAddress = cart.length > 0 ? cart[0].shipping_address : ''
                ...
                {cart.map((item) => (
                  <li key={item.id} className="flex px-4 py-6 sm:px-6">
                    <div className="flex-shrink-0">
                      <img src={`/img/${item.fabric}.jpg`} alt={item.shirt_type} className="h-20 w-20 rounded-md" />
                    </div>
